...
public:
     ...
     */
    const bool operator<(const CDate &t) const
    {
        return std::tie(year, month, day) < std::tie(t.year, t.month, t.day);
    }
...
public:
    // default constructor
    CSupermarket &store(const string &name, const CDate &expiry_date, const int count);
    friend bool getRealName(CSupermarket &t, string &product_validate);
...
bool getRealName(CSupermarket &obj, string &product_validate)
{
    ...
    for (const auto &map_element : obj.product_list) // iterating throught map elements
    {
 ...
 */
CSupermarket &CSupermarket::store(const string &name, const CDate &expiry_date, const int count) // TODO: maybe remove const &
{
...
list<pair<string, int>> CSupermarket::expired(const CDate &date) const
{
    ...
    for (auto const &product : product_list) // iterating through map
    {
        ...
        for (auto set_it = product.second.begin(); set_it != product.second.end(); ++set_it) // iterating through set
        {
